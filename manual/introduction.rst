...
멀티플 디스패치란게 필요치 않겠지만 줄리아의 특징들을 이어주는 핵심은 바로:
...
메서드 정의만 추가하면 된다. 기존 코드는 새로운 데이터 타입과 원할하게 작동한다.

런타임 타입 추론(타입 지시를 선택적으로 추가하여)을 이유로
또 프로젝트의 시발점에서 그 초점을 성능으로 하였기에
줄리아의 계산 효율은 다른 동적 언어들에 비해 우월하며
심지어 정적으로 컴파일된 경쟁 언어들마저 능가한다.
대규모의 수치 해석 문제들의 경우 속도는 매번 그리고
앞으로도 계속, 아마 항상 결정적 요소일 것이다:
